...
function EUF_TargetInfo_OnEvent(self, event, ...) 
    ...
    if event == "PLAYER_TARGET_CHANGED" then
        --EN_Msg("EUF_TargetInfo_OnEvent: "..event);
        EUF_TargetInfoClass_Update();
        EUF_TOTSELF_SOUND = 0;
    elseif event == "ADDON_LOADED" and name == EUF_SysAddonName then
        --EN_Msg("EUF_TargetInfo_OnEvent: "..event);
...
function EUF_TargetInfoTargetHPMP_Update(self, button)
        --DEBUG
        --print("EN_TargetInfo.lua, UnitPowerType: "..tostring(UnitPowerType("targettarget")));
    ...
    EUF_TargetInfoTargetMPBar:SetMinMaxValues(0, UnitPowerMax("targettarget"));
    EUF_TargetInfoTargetMPBar:SetValue(UnitPower("targettarget"));
...
function EUF_TargetFrameToTSound_Play()
    if EUF_CurrentOptions["TARGETOTWARNING"] == 1 and (GetNumSubgroupMembers() > 0 or IsInRaid()) then
...
function EUF_GetUnitInfoString(unit, withLevel, withLevelTag, withRace, withClass, withElite)
    ...
    if withClass == 1 then
        ...
        if class and UnitIsPlayer(unit) then
            tempstring = tempstring .. classLoc;
        end;
    end;
