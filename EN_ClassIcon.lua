...
local EUF_CLASS_TEXTURE = "Interface\\AddOns\\EN_UnitFrames\\EN_ClassIconLegion";
local EUF_CLASS_TEXTURE_SMALL = "Interface\\AddOns\\EN_UnitFrames\\EN_ClassIcon";
...
function EUF_SetClass(portrait, unit)
    ...
    if classLoc then
        ...
        if EUF_CurrentOptions[string.upper(category) .. "CLASSICONSMALL"] == 1 then
            if class and UnitIsPlayer(unit) then
                EUF_SetPortraitTextureSmall(getglobal("EUF_" .. category .. "Frame" .. categoryId .. "PortraitIcon"), class);
...
function EUF_SetPortraitTexture(portrait, class)
    --EN_Msg("PortraitName: "..tostring(portrait));
    --EN_Msg("Class: "..tostring(class));
    -- Set 8 class icon
    if EUF_CLASS_ICON[class] then
...
function EUF_SetPortraitTextureSmall(portrait, class)
    -- Set 8 class icon
    if EUF_CLASS_ICON[class] then
        portrait:SetTexture(EUF_CLASS_TEXTURE_SMALL);
        portrait:SetTexCoord(EUF_CLASS_ICON[class][1],EUF_CLASS_ICON[class][2],EUF_CLASS_ICON[class][3],EUF_CLASS_ICON[class][4]);
    else
        portrait:SetTexture("\\Interface\\CHARACTERFRAME\\TempPortrait");
    end;
...
function EUF_ClassIcon_Update(unit, iconType, value)
    ...
    if EUF_CurrentOptions["PLAYERCLASSICONBIG"] == 0 then
        if UnitHasVehicleUI("player") then
            ...
            SetPortraitTexture(PetPortrait, "player");
            SetPortraitTexture(PlayerPortrait, "pet");
        else
            PlayerPortrait:SetTexCoord(0, 1, 0, 1);
...
function EUF_UnitFrame_OnEvent(self, event, ...)
    if ((event == "UNIT_PORTRAIT_UPDATE") and (arg1 == self.unit)) then
        --EN_Msg("EUF_UnitFrame_OnEvent: "..event);
